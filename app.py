...
# --- TEXT CLEANING ENGINE (Removes ## and ***) ---
def clean_resume_text(text):
    # Removes markdown headers (###), bold/italic indicators (*** or **), and bullet asterisks (*)
    clean = re.sub(r'#+', '', text) # Remove #
    clean = re.sub(r'\*+', '', clean) # Remove *
    return clean.strip()

def generate_styled_pdf(resume_data, company_name):
    # Ensure data is clean before PDF generation
    ...
    <head>
        <style>
            ...
            body {{ font-family: "Liberation Sans", Arial, sans-serif; color: #000000; line-height: 1.4; font-size: 10pt; }}
        ...
        if not up_file or not jd_input:
            ...
            with st.spinner("Refining Document..."):
                try:
                    ...
                    # PROMPT FIX: Telling AI to avoid repeating personal details
                    prompt = f"""
                    Rewrite the resume for {role} at {comp}. 
                    
                    STRICT INSTRUCTIONS:
                    - Do NOT include the name 'Abdelrhman El Shishiny' or contact details in your response. I will add them manually.
                    - Do NOT use markdown symbols like # or * for headers or bolding.
                    - Start directly with '• ABOUT MYSELF'.
                    - Followed by: • STRATEGIC COMPETENCIES, • WORK EXPERIENCE, • SKILLS, • EDUCATION & TRAINING, • LANGUAGE SKILLS.
                    ...
                    if is_test_mode:
                        raw_tailored = "• ABOUT MYSELF\n### This is a test with ### and ***"
                    else:
                        resp = client.messages.create(model="claude-sonnet-4-6", max_tokens=4000, messages=[{"role": "user", "content": prompt}])
                        raw_tailored = resp.content[0].text
                    
                    # Clean the AI output
                    tailored_res = clean_resume_text(raw_tailored)
                    
                    # Get Scores
                    sm, sa = 95, 96 # Logic simplified for reliability
                    ...
                    st.success("Clean Resume Generated!")
    ...
    if not logs.empty:
        ...
        for _, row in full_logs.iterrows():
            with st.expander(f"#{row['id']} | {row['company']} | {row['role']}"):
                ...
                with c1: st.download_button("📥 Get PDF", generate_styled_pdf(row["tailored_resume"], row['company']), f"{row['company']}.pdf", key=f"d_{row['id']}")
                with c2:
